...
const Movies = () => {
  ...
  const { data: trendingMoviesData } = useGetTrendingMoviesQuery({ trending });

  return (
    <div className="flex flex-col justify-center items-center space-y-7 min-h-screen w-full">
      <HeroBanner pageId={pageId} />
      <h1 className="text-4xl text-white font-semibold mb-3">Movies</h1>
      ...
      <div className="flex flex-col overflow-x-auto w-full p-3">
        <div className="flex space-x-4">
          {trendingMoviesData?.results?.map((movie, idx) => (
            <SimilarMovieCard key={idx} {...movie} />
          ))}
        </div>
      </div>
        ...
        {isMoviesLoading ? (
          ...
          <div className="grid grid-cols-1 gap-y-3 sm:grid-cols-2 sm:gap-3 lg:grid-cols-4 xl:grid-cols-5 w-full md:gap-4 my-7">
            {moviesData?.results?.map((movie, idx) => (
              <MoviesCard path={path} key={idx} {...movie} />
            ))}
          </div>
        )}
      </div>
      <Pagination pages={moviesData?.total_pages} />
    </div>
  );
};
